...
public class ImageRouter_GET_Test extends ImageRouterTest {
    ...
    public void testContent(TestContext context) {
        ...
        doAnswer(TestUtils.createAsyncResultAnswer(sar)).when(imageApiMock).getContent(eq(image.getName()), any());

        Async getContent = context.async();
        getClient().getNow("/v1/images/" + image.getName() + "/content", response -> {
    ...
    public void testResourceNotFound(TestContext context) {
        SimpleAsyncResult<Image> sar = new SimpleAsyncResult<Image>(new BasicError(ErrorType.RESOURCE_NOT_FOUND));
