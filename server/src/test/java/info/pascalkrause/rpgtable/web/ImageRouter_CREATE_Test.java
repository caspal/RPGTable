...
public class ImageRouter_CREATE_Test extends ImageRouterTest {
    ...
    public void testWithName(TestContext context) {
        ...
        doAnswer(TestUtils.createAsyncResultAnswer(sar)).when(imageApiMock).create(eq(expected.getName()), eq(paylaod),
                any());
    ...
    public void testWithoutName(TestContext context) {
        ...
        })).when(imageApiMock).create(any(), eq(paylaod), any());
    ...
    public void testThrowRequestBodyTooLargeError(TestContext context) {
        Buffer paylaod = Buffer.buffer(SecureRandom.getSeed(config.BODY_SIZE_LIMIT_BYTES + 1));
        ...
        getClient().post("/v1/images", response -> {
            response.exceptionHandler(context.exceptionHandler());
            response.bodyHandler(body -> {
                TestUtils.isErrorResponseCorrect(response, body, new RequestBodyTooLargeError(paylaod.length(), config.BODY_SIZE_LIMIT_BYTES));
                createImage.complete();
            });
        }).putHeader(HttpHeaders.CONTENT_TYPE, MediaType.OCTET_STREAM.toString())
                .putHeader(HttpHeaders.CONTENT_LENGTH, Integer.toString(paylaod.length())).write(paylaod).end();
