...
import info.pascalkrause.rpgtable.web.ErrorResponse;
...
public class TestUtils {
    ...
    public static void isErrorResponseCorrect(HttpClientResponse response, Buffer body, ErrorResponse expected) {
        String expectedAsString = Json.encodePrettily(expected);

        assertThat(response.statusCode()).isEqualTo(expected.getStatusCode());
        assertThat(response.headers()).hasSize(2);
        int contentLength = Integer.parseInt(response.headers().get(HttpHeaders.CONTENT_LENGTH));
        assertThat(contentLength).isEqualTo(expectedAsString.length());
        assertThat(response.headers().get(HttpHeaders.CONTENT_TYPE)).isEqualTo(MediaType.JSON_UTF_8.toString());

        // Remove timestamp because they are never the same in two different error responses
        String timestampKey = "timestamp";
        JsonObject expectedAsJson = Buffer.buffer(expectedAsString).toJsonObject();
