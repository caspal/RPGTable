...
public class MongoImageStoreTestSuite {
    ...
    public void tearUp(TestContext testContext) throws IOException {
        ...
        testClass = new MongoImageStore("test_images", new MongoClientDataSource(mc), testContext.asyncAssertSuccess());
    }
    ...
    public void test(TestContext testContext) {
        ...
        initializeSuites("TestSuite MongoImageStore").forEach(suite -> {
            suite.getSuite().run(vertx, TestUtils.getTestOptions(vertx)).awaitSuccess(TimeUnit.SECONDS.toMillis(200000));
        });
