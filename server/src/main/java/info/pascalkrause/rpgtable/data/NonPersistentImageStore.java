...
public class NonPersistentImageStore implements ImageStore {
    ...
    public NonPersistentImageStore(FileSystem fs, String storageDir) {
        this.fs = fs;
    ...
    public void create(String name, Buffer buffer, Handler<AsyncResult<Image>> handler) {
        ...
        fs.open(path, new OpenOptions(), openFuture.completer());

        openFuture.compose(asyncFile -> {
            asyncFile.write(buffer);
            asyncFile.close(closeFuture.completer());
        }, closeFuture);
    }
    ...
    public void delete(String nameOrId, Handler<AsyncResult<Void>> handler) {
        get(nameOrId, res -> {
            ...
            if (!sameHash.isPresent()) {
                fs.delete(files.get(sameHash.get().hashCode()), v -> {
                    handler.handle(v);
                });
            }
        });
