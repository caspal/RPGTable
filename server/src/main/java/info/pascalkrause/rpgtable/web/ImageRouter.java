...
public class ImageRouter {
    ...
    private void handleFailure(RoutingContext rc) {
        if (!(rc.failure() instanceof BasicError)) {
            ...
            sendErrorResponse(ErrorResponse.UNEXPECTED_ERROR, rc);
            return;
        }
        ...
        case IMAGE_CREATE_EMPTY_BODY:
            sendErrorResponse(
                    new ErrorResponse(400, "[400-IMAGE-BOISEM]", "Body Is Empty", "There is no image in the body"), rc);
            break;
        case RESOURCE_ALREADY_EXIST:
            sendErrorResponse(new ErrorResponse(409, "[409-IMAGE-IMALEX]", "Image Already Exists",
                    "An image with name " + extractNameOrId(rc) + " already exist"), rc);
            break;
    ...
    private void list(RoutingContext routingContext) {
        imageApi.list(res -> {
            sendJsonResponse(200, res.result(), routingContext.response());
        });
    }
